...
void Accelero_Sensor_Handler( void *handle, uint32_t msTick, uint32_t *msTickStateChange, uint8_t *state )
{
  ...
  if ( status == 1 )
  {
    ...
    if(SendOverUSB) /* Write data on the USB */
    {
    	//convert the in32_t data type to enable floating point computation
        ...
        //transmit the message containing the Cartesian representation of the acceleration vector over USB
        ...
        //transmit some info about the current A_z value and state tracking variables
